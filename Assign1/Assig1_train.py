# %%
#1st Cell: Initialize Model to Be used for Training and Testing
                ...
                for i in range(img.shape[0]):
                    ...
                    for j in range(img.shape[1]):
                        ...
                        for k in range(img.shape[2]):
                            pixel.append(img[i][j][k] / 255.0)
                        row.append(pixel)
                    img_normalized.append(row)
                img = img_normalized

            if len(img) == detected_size and len(img[0]) == detected_size and isinstance(img[0][0], (int, float)):
                img = [[img]]
            else:
                ...
                for c in range(channels):
                    ...
                    for h in range(height):
                        ...
                        for w in range(width):
                            row.append(img[h][w][c])
                        channel.append(row)
                    transposed.append(channel)
                img = transposed
